...
export default function VideoTabUser({ props }) {
    ...
    useEffect(() => {
        ...
        database.ref(`Users/${props.uid}/Videos`).orderByChild("timestamp").limitToLast(9).on("value", (snapshot) => {
            let videoList = [];
            let flag = true;
            let key = ""
    ...
    const fetch = () => {
        setFetching(true)
        database.ref(`Users/${props.uid}/Videos`).orderByChild("timestamp").endBefore(lastKey).limitToLast(9).on("value", (snapshot) => {
            let videoList = [];
            let flag = true;
            let key = ""
