...
export default function FeedTabUser({ props }) {
    ...
    useEffect(() => {
        database.ref(`Users/${props.uid}/Posts`).orderByChild("timestamp").limitToLast(5).on("value", (snapshot) => {
            let postList = [];
            let flag = true;
            let key = ""
    ...
    const fetch = () => {
        setFetching(true)
        database.ref(`Users/${props.uid}/Posts`).orderByChild("timestamp").endBefore(lastKey).limitToLast(5).on("value", (snapshot) => {
            let postList = [];
            let flag = true;
            let key = ""
            snapshot.forEach((snap) => {
